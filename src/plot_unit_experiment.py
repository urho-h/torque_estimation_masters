...
def plot_step_experiment(plot_kf_vs_tikh=False, plot_lasso_vs_tikh=False):
    ...
    if plot_kf_vs_tikh:
        ...
        plt.plot(times, inputs_tikh[::2], label='H-P trend filter', alpha=0.8)
        plt.tick_params('x', labelbottom=False)

        plt.subplot(312)
        plt.plot(times, propeller, label='Measurement')
        plt.plot(times[:-10], input_estimates_kf[10:,1], label='Kalman filter')
        plt.plot(times, inputs_tikh[1::2], label='H-P trend filter', alpha=0.8)
        ...
        plt.plot(times, states_tikh[:,-1], label='Tikhonov regularization', alpha=0.8)
        plt.xlabel('Time (s)')
    ...
    if plot_lasso_vs_tikh:
        ...
        plt.subplot(311)
        plt.plot(times, motor, label='Measurement')
        plt.plot(times, inputs_tikh[::2], label='Tikhonov regularization', alpha=0.8)
        plt.plot(times, inputs_lasso[::2], label='$\ell_1$-regularization', alpha=0.8)
        plt.tick_params('x', labelbottom=False)
        # plt.xlim(5,5.5)

        plt.subplot(312)
        plt.plot(times, propeller, label='Measurement')
        plt.plot(times, inputs_tikh[1::2], label='Tikhonov regularization', alpha=0.8)
        plt.plot(times, inputs_lasso[1::2], label='$\ell_1$-regularization', alpha=0.8)
        plt.ylabel('Torque (Nm)')
        plt.tick_params('x', labelbottom=False)
        # plt.xlim(5,5.5)
        ...
        plt.subplot(313)
        plt.plot(times, torques[:,-1], label='Measurement')
        plt.plot(times, states_tikh[:,-1], label='Tikhonov', alpha=0.8)
        plt.plot(times, states_lasso[:,-1], label='$\ell_1$', alpha=0.8)
        plt.xlabel('Time (s)')
        plt.xlim(5,5.5)
        ...
        plt.figure()
        plt.plot(times, propeller, label='Measurement')
        plt.plot(times, inputs_tikh[1::2], label='Tikhonov')
        plt.plot(times, inputs_lasso[1::2], label='$\ell_1$')
        plt.ylabel('Torque (Nm)')
        plt.xlabel('Time (s)')
        plt.xlim(5.1,5.22)
        plt.ylim(0,1.5)
...
if __name__ == "__main__":
    plot_step_experiment(plot_kf_vs_tikh=False, plot_lasso_vs_tikh=True)
